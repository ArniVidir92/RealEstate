...
Radj.allYTransformed <- CalculateRadjusted(lm.allYTransformed, testTransf) # 0.8488961
...
Radj.allYTransformedAndIBM2 <- CalculateRadjusted(lm.allYTransformedAndIBM2, testTransf) # 0.8880478

# use step function going backwards
slm.stepBackYTransformedAndIBM2 <- step(lm.allYTransformedAndIBM2)
Radj.stepBackYTransformedAndIBM2 <- CalculateRadjusted(slm.stepBackYTransformedAndIBM2, testTransf) # 0.889269


# use step function going forward
slm.stepForwYTransformedAndIBM2 <- step(lm.allYTransformedAndIBM2, direction = 'forward')
Radj.stepForwYTransformedAndIBM2 <- CalculateRadjusted(slm.stepForwYTransformedAndIBM2, testTransf) # 0.8880478
...
Radj.stepTest <- CalculateRadjusted(slm.stepTest, testTransf) # 0.889269
...
Radj.BICStep <- CalculateRadjusted(lm.BIC, testTransf) # 0.8889675

# AIC test
lm.AIC <- step(null, scope = list(lower=null,upper=full),
     direction="both", k=2)
Radj.AICStep <- CalculateRadjusted(lm.AIC, testTransf) # 0.889269

# Ath. virkar ekki. langaði að fjarlægja stærstu outliers og sjá hvort við fáum betra Radj.
#ind <- RemoveNOutliers(train,4)
